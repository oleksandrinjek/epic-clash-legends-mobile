-- Fix Epic Clash Legends Mobile Database Schema
-- This script updates the schema to use string IDs instead of UUIDs

-- Drop existing tables (if they exist)
DROP TABLE IF EXISTS game_sessions CASCADE;
DROP TABLE IF EXISTS player_heroes CASCADE;
...
-- Player progress table with string player_id
...
-- Insert a test player to verify the setup
INSERT INTO players (id, name) VALUES 
  ('player_1756070658790_zi1jjxzgz', 'Test Player')
ON CONFLICT (id) DO NOTHING;

-- Insert test progress for the player
INSERT INTO player_progress (player_id, level, experience, coins, wins, losses) VALUES 
  ('player_1756070658790_zi1jjxzgz', 1, 0, 1250, 0, 0)
ON CONFLICT (player_id) DO NOTHING;

-- Verify the setup
SELECT 'Players table:' as table_name, count(*) as record_count FROM players
UNION ALL
SELECT 'Player progress table:', count(*) FROM player_progress;
